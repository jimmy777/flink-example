 ...
 * @Description 分流器。定义 outputTag -> connect() -> getSideOutput() 获取分流数据。
 *
 * 可以分流往不同的目的如 hbase 或者 es；
 *
 * 注意：
 * 1. outputTag 可以定义一个分流器 OutputTag<T>；
 * 2. 使用 ctx.output(ot, object) 来发射数据，而不是 collect() 来发射；
 * 3. 使用 getSideOutput() 来接收分流的数据。
 *
 */
public class OutputTagOperator {
